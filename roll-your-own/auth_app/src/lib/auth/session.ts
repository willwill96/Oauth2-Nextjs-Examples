...
async function executeBackchannelLogout(id_token: string) {
    return fetch(`${process.env.KEYCLOAK_ISSUER}/protocol/openid-connect/logout?id_token_hint=${id_token}`)
}
...
function tokensToRedisSession(tokens: OAuth2Tokens): [BackendSession, Date] {
    ...
    const parsedAccessToken = decodeJWT(access_token) as { exp: number, email: string }
...
export async function createSession(tokens: OAuth2Tokens): Promise<BackendSessionWithId> {
    ...
    await client.expire(`session:${sessionId}`, Math.floor((expiryDate.getTime() - Date.now()) / 1000))
    await setSessionCookie(sessionToken, expiryDate)
    return { session_id: sessionId, ...baseSession}
}

export async function updateSession(tokens: OAuth2Tokens, sessionToken: string, sessionId: string): Promise<BackendSessionWithId> {
    const [baseSession, expiryDate] = tokensToRedisSession(tokens)
    await client.hSet(`session:${sessionId}`, baseSession);
    await client.expire(`session:${sessionId}`, Math.floor((expiryDate.getTime() - Date.now()) / 1000))
    await setSessionCookie(sessionToken, expiryDate)
...
async function refreshTokens(session: BackendSession, sessionToken: string, sessionId: string) {
    try {
